...
class ProcessData implements DataFunctions
{
    ...
    public function validateData($record, $callingPage)
    {
        ...
        if (empty($record['firstName'])) {
            ...
            ++$i;
        } elseif (ctype_alpha($record['firstName']) === false) {
        ...
        if (empty($record['lastName'])) {
            ...
            ++$i;
        } elseif (ctype_alpha($record['lastName']) === false) {
            $errors[$i] = 'A name must contain only characters';
            ++$i;
        }
        ...
        if (empty($record['city'])) {
            ...
            ++$i;
        } elseif (ctype_alpha($record['city']) === false) {
        ...
        if (empty($record['state'])) {
            ...
            ++$i;
        } elseif (ctype_alpha($record['state']) === false) {
        ...
        if (empty($record['zip'])) {
            ...
            ++$i;
        } elseif (ctype_digit($record['zip']) === false) {
        ...
        if (empty($record['telephone'])) {
            ...
            ++$i;
        } elseif (ctype_digit($record['telephone']) === false) {
            $errors[$i] = 'A telephone number must contain only digits';
            ++$i;
        } elseif (!(strlen($record['telephone']) == 10)) {
        ...
        if (empty($record['mobile'])) {
            ...
            ++$i;
        } elseif (ctype_digit($record['mobile']) === false) {
            $errors[$i] = 'A mobile number must contain only digits';
            ++$i;
        } elseif (!(strlen($record['mobile']) == 10)) {
    ...
    public function checkUniqueness($element, $elementValue, $callingPage, $origin = 'nonAjax')
    {
        global $connection;
